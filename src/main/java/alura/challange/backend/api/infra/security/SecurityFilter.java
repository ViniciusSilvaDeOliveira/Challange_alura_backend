...
import alura.challange.backend.api.domain.usuario.UsuarioRepository;
...
public class SecurityFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (tokenJWT != null){
            ...
            var usuario = repository.findByLogin(subject); //recupera o objeto usuario
            var authentication = new UsernamePasswordAuthenticationToken(usuario, null, usuario.getAuthorities());
            SecurityContextHolder.getContext().setAuthentication(authentication);//força a autenticação
        }

        filterChain.doFilter(request, response); //verificar se a pessoa está logada
    }
